  ...
  @if($crud->getOperationSetting('resetButton') ?? true)
    ...
    $('#crudTable_reset_button').on('click', function() {
      ...
      if (localStorage.getItem('{{ Str::slug($crud->getRoute())}}_list_url_time')) {
        ...
        // Get the current section route and normalize it
        const crudRoute = '{{ $crud->getRoute() }}'.replace(/^\/|\/$/g, "");

        // Find and remove the correct DataTables LocalStorage key for this section
        for (let i = 0; i < localStorage.length; i++) {
            const key = localStorage.key(i);

            // Ensure we are targeting the exact DataTables key for this route
            if (key && key.startsWith('DataTables_crudTable_') && key.includes(`/${crudRoute}`)) {
