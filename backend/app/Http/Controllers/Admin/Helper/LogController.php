	...
	public function export(Request $request)
	{
		...
		if ($fileSize > $absoluteMaxSize) {
			abort(
				...
				"File too large (" .
					...
					"). Maximum size: " .
					$this->formatBytes($absoluteMaxSize) .
					". Please archive or rotate this log file."
			);
		}
		...
		if ($fileSize > $maxFileSize) {
			...
			if (!$levelFilter && !$searchFilter) {
				$logContent =
					"[WARNING] File truncated to last " .
					number_format($maxLines) .
					" lines due to size (" .
					$this->formatBytes($fileSize) .
					")\n\n" .
					$logContent;
	...
	private function readLastLines($filePath, $maxLines = 50000): string
	{
		...
		if ($this->encryptionService->isEncrypted($sample)) {
			...
			try {
				...
				return "[ERROR] Could not decrypt large file: " .
					$e->getMessage() .
					"\n[INFO] File size: " .
					$this->formatBytes($fileSize) .
