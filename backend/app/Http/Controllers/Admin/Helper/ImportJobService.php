<?php

namespace App\Http\Controllers\Admin\Helper;

use Illuminate\Support\Str;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Facades\Schema;
use App\Http\Controllers\Admin\Helper\Core\CsvHelper;
use App\Http\Controllers\Admin\Helper\Core\CsvBackupManager;
use App\Http\Controllers\Admin\Helper\Core\CsvTransformerJob;
use App\Models\Tiraggio;
use App\Notifications\UserCloseTiraggioNotification;
use App\Notifications\FilialeCloseTiraggioNotification;
use Carbon\Carbon;

/**
 * Import Job Service - Handles CSV import for automated jobs
 * Optimized for cron job imports with logging and error handling
 */
class ImportJobService
{
	/**
	 * Import CSV file for job
	 *
	 * @param string $fileName File name
	 * @param string $subfolder Subfolder name
	 * @param string $modelClass Model class name
	 * @param string|null $uniqueField Unique field for upsert (optional)
	 * @return array Import statistics
	 */
	public static function import(
		string $fileName,
		string $subfolder,
		string $modelClass,
		?string $uniqueField = null
	): array {
		Log::channel("import")->info("Import file in import/" . $subfolder . "/" . $fileName . " avviato alle " . now());

		// Increase execution time and memory for large imports
		set_time_limit(0);
		ini_set("memory_limit", "1G");

		// Set database timeout for large batch operations
		config([
			"database.connections.mysql.options" => [
				\PDO::ATTR_TIMEOUT => 300,
			],
		]);

		$csvPath = database_path("import/{$subfolder}/{$fileName}");

		if (!file_exists($csvPath)) {
			throw new \Exception("CSV file not found: {$csvPath}");
		}

		try {
			$model = new $modelClass();
			$tableName = $model->getTable();

			// Create backup before import
			Log::channel("import")->info("Creating backup for table " . $tableName);
			CsvBackupManager::createBackup($tableName, "job", true);
			Log::channel("import")->info("Backup for table " . $tableName . " created");

			DB::statement("SET FOREIGN_KEY_CHECKS=1");

			$result = self::performImport($csvPath, $modelClass, $uniqueField);

			// Hash passwords if importing User model
			if (str_contains($modelClass, "User")) {
				Log::channel("import")->info("Hashing user passwords");
				ImportSeederService::hashUserPasswords(true);
				Log::channel("import")->info("User passwords hashed");
			}

			return $result;
		} catch (\Exception $e) {
			Log::channel("import")->error("Import failed: " . $e->getMessage());
			throw $e;
		}
	}

	/**
	 * Perform the actual import operation
	 *
	 * @param string $csvPath Full path to CSV file
	 * @param string $modelClass Model class name
	 * @param string|null $uniqueField Unique field for upsert
	 * @return array Import statistics
	 */
	private static function performImport(string $csvPath, string $modelClass, ?string $uniqueField = null): array
	{
		$delimiter = CsvHelper::detectDelimiter($csvPath);
		$handle = fopen($csvPath, "r");

		if (!$handle) {
			throw new \Exception("Unable to open CSV file: {$csvPath}");
		}

		// Read headers
		$headers = fgetcsv($handle, 0, $delimiter);
		if (!$headers) {
			fclose($handle);
			throw new \Exception("Invalid CSV format: no headers found");
		}

		// Get model information
		$model = new $modelClass();
		$tableColumns = Schema::getColumnListing($model->getTable());
		$fillableColumns = $model->getFillable();

		// Create column mapping automatically
		$columnMapping = self::createAutoMapping($headers, $tableColumns, $fillableColumns);

		// if (empty($columnMapping)) {
		// 	fclose($handle);
		// 	throw new \Exception("No valid column mappings found between CSV and model");
		// }

		// No cache - using direct DB queries instead

		// Count total rows for progress (optimized for large files)
		$fileSize = filesize($csvPath);
		$totalRows =
			$fileSize > 100 * 1024 * 1024 ? intval($fileSize / 150) : CsvHelper::countCsvRows($csvPath, $delimiter) - 1;

		// Statistics
		$stats = [
			"created" => 0,
			"updated" => 0,
			"skipped" => 0,
			"errors" => 0,
			"processed" => 0,
		];

		// Batch configuration
		$batchSize = 200;
		$batch = [];
		$lastProgress = -1;
		$processedInTransaction = 0;
		$maxTransactionRows = 5000;

		Log::channel("import")->info("Processing {$totalRows} rows in batches of {$batchSize}");

		// MySQL optimizations
		try {
			DB::statement("SET FOREIGN_KEY_CHECKS=0");
			DB::statement("SET UNIQUE_CHECKS=0");
		} catch (\Exception $e) {
			Log::channel("import")->warning("Could not set MySQL optimizations: " . $e->getMessage());
		}

		DB::beginTransaction();

		try {
			while (($row = fgetcsv($handle, 0, $delimiter)) !== false) {
				$stats["processed"]++;

				// Map row data
				$rowData = self::mapRowData($row, $columnMapping ?? [], $headers, $modelClass);

				if (empty($rowData)) {
					$stats["skipped"]++;
					continue;
				}

				// Skip records marked for skip (e.g., missing user_ndg for Delibera)
				if (!empty($rowData["__skip_record"])) {
					// Reduced logging: only log skip reasons every 100 records to minimize disk I/O
					if ($stats["skipped"] % 100 == 0) {
						$skipReason = $rowData["__skip_reason"] ?? "Unknown reason";
						Log::channel("import")->warning(
							"Skipping records (sample): {$skipReason} (total skipped: {$stats["skipped"]})"
						);
					}
					$stats["skipped"]++;
					continue;
				}

				// Apply model-specific transformations
				$rowData = CsvTransformerJob::transform($rowData, $row, $columnMapping ?? [], $headers, $modelClass);

				if (empty($rowData)) {
					$stats["skipped"]++;
					continue;
				}

				// Check if this is a multiple records case (for Proiezione)
				if (isset($rowData["__multiple_records__"])) {
					foreach ($rowData["__multiple_records__"] as $record) {
						$batch[] = $record;
						$processedInTransaction++;
					}
				} else {
					$batch[] = $rowData;
					$processedInTransaction++;
				}

				// Process batch when full or at end of file
				if (count($batch) >= $batchSize || $stats["processed"] >= $totalRows) {
					$batchStats = self::processBatch($batch, $modelClass, null);

					$stats["created"] += $batchStats["created"];
					$stats["updated"] += $batchStats["updated"];
					$stats["skipped"] += $batchStats["skipped"];
					$stats["errors"] += $batchStats["errors"];

					$batch = [];

					// Commit transaction periodically
					if ($processedInTransaction >= $maxTransactionRows || $stats["processed"] >= $totalRows) {
						DB::commit();
						if ($stats["processed"] < $totalRows) {
							DB::beginTransaction();
						}
						$processedInTransaction = 0;
					}

					// Log progress every 25% (reduced frequency to minimize disk I/O)
					$progress = round((($stats["processed"] / $totalRows) * 100) / 25) * 25;
					if ($progress !== $lastProgress && $progress % 25 == 0 && $progress > 0) {
						Log::channel("import")->info(
							"Progress: {$progress}% ({$stats["processed"]}/{$totalRows}) - Memory: " .
								round(memory_get_usage(true) / 1024 / 1024, 2) .
								"MB"
						);
						$lastProgress = $progress;
					}

					// Force garbage collection periodically
					if ($stats["processed"] % 2000 == 0) {
						gc_collect_cycles();
					}
				}
			}

			DB::commit();

			// Handle Trattenuta-specific operations for job imports
			// Note: Trattenuta import should only insert trattenute, not update tiraggi
			// Tiraggio updates should be handled separately if needed
			if (str_contains($modelClass, "Trattenuta")) {
				Log::channel("trattenuta")->info("Trattenuta import completed - no tiraggio updates performed");
			}
		} catch (\Exception $e) {
			DB::rollBack();
			throw $e;
		} finally {
			// Restore MySQL settings
			try {
				DB::statement("SET FOREIGN_KEY_CHECKS=1");
				DB::statement("SET UNIQUE_CHECKS=1");
			} catch (\Exception $e) {
				// Ignore restore errors
			}
			fclose($handle);
		}

		return $stats;
	}

	/**
	 * Create automatic column mapping between CSV and database
	 *
	 * @param array $headers CSV headers
	 * @param array $tableColumns Database table columns
	 * @param array $fillableColumns Fillable columns from model
	 * @return array Column mapping (csv_index => db_column)
	 */
	private static function createAutoMapping(array $headers, array $tableColumns, array $fillableColumns): array
	{
		$mapping = [];

		foreach ($headers as $index => $header) {
			$normalizedHeader = Str::snake(strtolower(trim($header)));

			// Check if column exists in table and is fillable
			if (
				in_array($normalizedHeader, $tableColumns) &&
				(empty($fillableColumns) || in_array($normalizedHeader, $fillableColumns))
			) {
				$mapping[$index] = $normalizedHeader;
			}
		}

		return $mapping;
	}

	/**
	 * Map CSV row data to database columns
	 *
	 * @param array $row CSV row data
	 * @param array $columnMapping Column mapping
	 * @param array $headers CSV headers
	 * @param string $modelClass Model class name
	 * @return array Mapped row data
	 */
	private static function mapRowData(array $row, array $columnMapping = [], array $headers, string $modelClass): array
	{
		$rowData = [];

		$nonDateColumns = ["max_atp"];

		foreach ($columnMapping as $csvIndex => $dbColumn) {
			if (isset($row[$csvIndex])) {
				$value = trim($row[$csvIndex]);

				// Handle numeric values (preserving comma as decimal separator)
				if (!CsvHelper::isDateLikeColumn($dbColumn)) {
					if (CsvHelper::looksNumeric($value)) {
						$value = CsvHelper::simpleDecimal($value);
					}
				}

				// Handle special date formats
				if (
					str_contains($dbColumn, "_at") &&
					!in_array($dbColumn, $nonDateColumns, true) &&
					$value &&
					$value !== ""
				) {
					$parsedDate = CsvHelper::parseDate($value);
					if ($parsedDate) {
						$value = $parsedDate;
					}
				}

				// Handle empty values for numeric fields - convert to 0 instead of skipping
				if (
					$value === "" &&
					in_array($dbColumn, [
						"importo_richiesto",
						"costo_totale",
						"importo_transato",
						"numero_transazioni",
						"importo_previsto",
						"importo_deliberato",
						"trattenuta_totale",
						"importo_trattenuto",
						"mesi",
						"mese",
						"anno",
						"percentuale",
						"quota_interessi",
						"forecast",
						"baseline",
					])
				) {
					$rowData[$dbColumn] = 0;
				} elseif ($value !== "") {
					$rowData[$dbColumn] = $value;
				}
			}
		}

		// Add default values for missing required fields
		$rowData = self::addDefaultRequiredFields($rowData, $modelClass);

		return $rowData;
	}

	/**
	 * Add default values for missing required fields
	 *
	 * @param array $rowData Current row data
	 * @param string $modelClass Model class name
	 * @return array Row data with defaults added
	 */
	private static function addDefaultRequiredFields(array $rowData, string $modelClass): array
	{
		$className = class_basename($modelClass);

		switch ($className) {
			case "User":
				// Set default role_id for users if not present
				if (!isset($rowData["role_id"]) || empty($rowData["role_id"])) {
					$rowData["role_id"] = 2;
				}

				// Ensure email_verified_at is set
				if (!isset($rowData["email_verified_at"]) || empty($rowData["email_verified_at"])) {
					$rowData["email_verified_at"] = now();
				}
				break;
		}

		return $rowData;
	}

	/**
	 * Process a batch of records with optimized bulk operations
	 *
	 * @param array $batch Batch of records to process
	 * @param string $modelClass Model class name
	 * @param string|null $uniqueField Unique field for upsert
	 * @return array Batch processing statistics
	 */
	private static function processBatch(array $batch, string $modelClass, ?string $uniqueField = null): array
	{
		$stats = ["created" => 0, "updated" => 0, "skipped" => 0, "errors" => 0];

		if (empty($batch)) {
			return $stats;
		}

		// For job imports: implement upsert logic (update if exists, insert if not)
		try {
			$model = new $modelClass();
			$tableName = $model->getTable();
			$className = class_basename($modelClass);

			// Add timestamps if model uses them
			if ($model->usesTimestamps()) {
				$now = now();
				foreach ($batch as &$item) {
					$item["created_at"] = $item["created_at"] ?? $now;
					$item["updated_at"] = $item["updated_at"] ?? $now;
				}
			}

			// Special handling for Trattenuta: bulk insert for better performance
			if ($className === "Trattenuta" && !empty($batch)) {
				// Clean batch: remove internal fields
				$cleanBatch = [];
				foreach ($batch as $rowData) {
					// Remove internal fields that shouldn't be inserted
					unset($rowData["__tiraggio_update__"]);
					unset($rowData["__skip_record"]);
					unset($rowData["__skip_reason"]);
					unset($rowData["__existing_record_id"]);
					$cleanBatch[] = $rowData;
				}

				if (!empty($cleanBatch)) {
					try {
						// Get column names from first row
						$columns = array_keys($cleanBatch[0]);
						$columnList = "`" . implode("`, `", $columns) . "`";

						// Build VALUES clause with placeholders
						$placeholders = "(" . implode(", ", array_fill(0, count($columns), "?")) . ")";
						$valuesClause = implode(", ", array_fill(0, count($cleanBatch), $placeholders));

						// Flatten all values in correct order
						$allValues = [];
						foreach ($cleanBatch as $row) {
							foreach ($columns as $column) {
								$allValues[] = $row[$column] ?? null;
							}
						}

						// Execute bulk INSERT IGNORE (ignores duplicates)
						DB::statement(
							"INSERT IGNORE INTO `{$tableName}` ({$columnList}) VALUES {$valuesClause}",
							$allValues
						);
						$stats["created"] += count($cleanBatch);
					} catch (\Exception $e) {
						// Fallback to individual inserts if bulk fails
						Log::channel("import")->warning(
							"Bulk insert failed for Trattenuta, falling back to individual inserts: " . $e->getMessage()
						);
						foreach ($cleanBatch as $rowData) {
							try {
								$modelClass::create($rowData);
								$stats["created"]++;
							} catch (\Exception $e2) {
								$stats["errors"]++;
								Log::channel("import")->warning("Failed to insert Trattenuta: " . $e2->getMessage());
							}
						}
					}
				}
				return $stats;
			}

			// Process each record individually with model-specific logic
			foreach ($batch as $rowData) {
				try {
					$className = class_basename($modelClass);

					// Handle explicit existing record override (used by Delibera to match user/status)
					if (!empty($rowData["__existing_record_id"])) {
						$existingId = $rowData["__existing_record_id"];
						unset($rowData["__existing_record_id"]);

						$existingRecord = $modelClass::find($existingId);
						if ($existingRecord) {
							$existingRecord->update($rowData);
							$stats["updated"]++;

							// Update open delibera cache if this is a Delibera update
							if ($className === "Delibera" && isset($rowData["user_ndg"])) {
								\App\Http\Controllers\Admin\Helper\Core\CsvTransformerJob::updateOpenDeliberaCache(
									$rowData["user_ndg"]
								);
							}

							continue;
						}
					}

					// Special logic for Trattenuta: bulk insert for better performance
					if ($className === "Trattenuta") {
						// Skip individual processing - will be handled in bulk below
						continue;
					}

					// Special logic for Mensile: use ndg_year_month as unique field
					if (
						$className === "Mensile" &&
						isset($rowData["ndg_year_month"]) &&
						!empty($rowData["ndg_year_month"])
					) {
						$existingRecord = $modelClass::where("ndg_year_month", $rowData["ndg_year_month"])->first();
						if ($existingRecord) {
							// Update existing record
							$existingRecord->update($rowData);
							$stats["updated"]++;
						} else {
							// Insert new record
							$modelClass::create($rowData);
							$stats["created"]++;
						}
						continue;
					}

					// Special logic for Tiraggio: skip upsert logic if __existing_record_id is not set
					// This allows simple tiraggio imports (open status) to create new records
					if ($className === "Tiraggio" && empty($rowData["__existing_record_id"])) {
						// For Tiraggio without explicit existing record, just insert
						// The transform logic already handles finding/updating existing open tiraggi
						try {
							$newRecord = $modelClass::create($rowData);
							$stats["created"]++;
							Log::channel("import")->info(
								"Tiraggio created: ID {$newRecord->id} for user_ndg " .
									($rowData["user_ndg"] ?? "N/A") .
									" with status {$rowData["status"]}"
							);
							Log::channel("tiraggio")->info(
								"Tiraggio created: ID {$newRecord->id} for user_ndg " .
									($rowData["user_ndg"] ?? "N/A") .
									" with status {$rowData["status"]}"
							);
						} catch (\Exception $e) {
							$stats["skipped"]++;
							Log::channel("import")->error(
								"Failed to create Tiraggio: " . $e->getMessage() . " | Data: " . json_encode($rowData)
							);
							Log::channel("tiraggio")->error(
								"Failed to create Tiraggio: " . $e->getMessage() . " | Data: " . json_encode($rowData)
							);
						}
						continue;
					}

					// For other models: use upsert logic
					$uniqueFieldToUse = self::getUniqueFieldForModel($modelClass, $rowData);

					if (!$uniqueFieldToUse || !isset($rowData[$uniqueFieldToUse])) {
						// No unique field or value, just insert
						$newRecord = $modelClass::create($rowData);
						$stats["created"]++;

						// Update open delibera cache if this is a Delibera creation
						if ($className === "Delibera" && isset($rowData["user_ndg"])) {
							\App\Http\Controllers\Admin\Helper\Core\CsvTransformerJob::updateOpenDeliberaCache(
								$rowData["user_ndg"]
							);
						}

						continue;
					}

					// Check if record exists with this unique value
					$existingRecord = self::findExistingRecord($modelClass, $rowData, $uniqueFieldToUse);

					if ($existingRecord) {
						// Update existing record
						$existingRecord->update($rowData);
						$stats["updated"]++;

						// Update open delibera cache if this is a Delibera update
						if ($className === "Delibera" && isset($rowData["user_ndg"])) {
							\App\Http\Controllers\Admin\Helper\Core\CsvTransformerJob::updateOpenDeliberaCache(
								$rowData["user_ndg"]
							);
						}
					} else {
						// Insert new record
						$newRecord = $modelClass::create($rowData);
						$stats["created"]++;

						// Update open delibera cache if this is a Delibera creation
						if ($className === "Delibera" && isset($rowData["user_ndg"])) {
							\App\Http\Controllers\Admin\Helper\Core\CsvTransformerJob::updateOpenDeliberaCache(
								$rowData["user_ndg"]
							);
						}
					}
				} catch (\Exception $e) {
					$stats["skipped"]++;
					Log::channel("import")->warning("Skipped row due to error: " . $e->getMessage());
				}
			}
		} catch (\Exception $e) {
			$stats["errors"] += count($batch);
			Log::channel("import")->error("Batch processing error: " . $e->getMessage());
		}

		return $stats;
	}

	/**
	 * Determine the unique field for a model based on database schema
	 *
	 * @param string $modelClass Model class name
	 * @param array $rowData Row data to check for unique fields
	 * @return string|null Unique field name or null if not found
	 */
	private static function getUniqueFieldForModel(string $modelClass, array $rowData): ?string
	{
		$model = new $modelClass();
		$tableName = $model->getTable();

		// Get unique indexes from database schema
		$uniqueFields = self::getUniqueFieldsFromSchema($tableName);

		if (empty($uniqueFields)) {
			return null;
		}

		// For single unique fields, return the first one that exists in rowData
		foreach ($uniqueFields as $field) {
			if (isset($rowData[$field]) && !empty($rowData[$field])) {
				return $field;
			}
		}

		return null;
	}

	/**
	 * Get unique fields from database schema
	 *
	 * @param string $tableName Table name
	 * @return array Array of unique field names
	 */
	private static function getUniqueFieldsFromSchema(string $tableName): array
	{
		try {
			// Query INFORMATION_SCHEMA to get unique indexes
			$uniqueIndexes = DB::select(
				"
				SELECT COLUMN_NAME 
				FROM INFORMATION_SCHEMA.STATISTICS 
				WHERE TABLE_SCHEMA = ? 
				AND TABLE_NAME = ? 
				AND NON_UNIQUE = 0 
				AND INDEX_NAME != 'PRIMARY'
				ORDER BY SEQ_IN_INDEX
			",
				[config("database.connections.mysql.database"), $tableName]
			);

			return array_column($uniqueIndexes, "COLUMN_NAME");
		} catch (\Exception $e) {
			Log::channel("import")->warning("Could not get unique fields for table {$tableName}: " . $e->getMessage());
			return [];
		}
	}

	/**
	 * Find existing record based on unique field(s)
	 *
	 * @param string $modelClass Model class name
	 * @param array $rowData Row data
	 * @param string $uniqueField Primary unique field
	 * @return \Illuminate\Database\Eloquent\Model|null
	 */
	private static function findExistingRecord(
		string $modelClass,
		array $rowData,
		string $uniqueField
	): ?\Illuminate\Database\Eloquent\Model {
		$model = new $modelClass();
		$tableName = $model->getTable();

		// Get all unique fields from schema
		$uniqueFields = self::getUniqueFieldsFromSchema($tableName);

		if (empty($uniqueFields)) {
			return null;
		}

		// Build query with all unique fields that have values in rowData
		$query = $modelClass::query();
		$hasConditions = false;

		foreach ($uniqueFields as $field) {
			if (isset($rowData[$field]) && !empty($rowData[$field])) {
				$query->where($field, $rowData[$field]);
				$hasConditions = true;
			}
		}

		// Only execute query if we have at least one condition
		return $hasConditions ? $query->first() : null;
	}

	/**
	 * Close tiraggi that have reached their importo_deliberato limit (direct DB query)
	 */
	private static function closeTiraggiAfterTrattenuta(): void
	{
		// Find all accepted tiraggi where trattenuta_totale >= importo_deliberato
		$tiraggiToClose = Tiraggio::where("status", "accepted")
			->whereRaw("trattenuta_totale >= importo_deliberato")
			->where("importo_deliberato", ">", 0)
			->get();

		if ($tiraggiToClose->isEmpty()) {
			echo "No tiraggi to close\n";
			return;
		}

		echo "Closing {$tiraggiToClose->count()} tiraggi...\n";

		foreach ($tiraggiToClose as $tiraggio) {
			// Refresh from DB to get ACTUAL current status
			$tiraggio->refresh();

			// CRITICAL: Check if tiraggio is already closed - skip if already paid
			if ($tiraggio->status === "paid") {
				Log::channel("trattenuta")->warning("Tiraggio {$tiraggio->id} is already closed (status: paid) - skipping");
				continue;
			}

			// Only process if status is still "accepted"
			if ($tiraggio->status !== "accepted") {
				Log::channel("trattenuta")->warning(
					"Tiraggio {$tiraggio->id} has unexpected status: {$tiraggio->status} - skipping"
				);
				continue;
			}

			// Check if user has filiale role - skip notifications
			$userRoleName = $tiraggio->user->role->name ?? null;
			$isFilialeUser = $userRoleName && strtolower($userRoleName) === "filiale";

			$oldTrattenuta = $tiraggio->trattenuta_totale;
			$delibera_numero_pratica = $tiraggio->delibera_numero_pratica;
			$tiraggio->status = "paid";
			$tiraggio->trattenuta_totale = $tiraggio->importo_deliberato;
			$tiraggio->data_fine = now();
			$tiraggio->save();
			$tiraggio->refresh();

			$tiraggio_open = Tiraggio::where("user_ndg", $tiraggio->user_ndg)->where("status", "open")->first();
			if (!$tiraggio_open) {
				$tiraggio_new = Tiraggio::create([
					"user_ndg" => $tiraggio->user_ndg,
					"status" => "open",
					"tan" => $tiraggio->tan ?? ($tiraggio->delibera->tan ?? null),
					"taeg" => $tiraggio->taeg ?? ($tiraggio->delibera->taeg ?? null),
					"delibera_numero_pratica" => $delibera_numero_pratica,
				]);
				Log::channel("trattenuta")->info(
					"Tiraggio {$tiraggio->id} closed - trattenuta: {$oldTrattenuta} → {$tiraggio->importo_deliberato} - Open new tiraggio {$tiraggio_new->id}"
				);
			} else {
				Log::channel("trattenuta")->info(
					"Tiraggio {$tiraggio->id} closed - trattenuta: {$oldTrattenuta} → {$tiraggio->importo_deliberato} - Tiraggio with status open already exists"
				);
			}

			// Send notifications ONLY when closing for the first time AND user is NOT filiale
			if (!$isFilialeUser) {
				// $tiraggio->user->notify(new UserCloseTiraggioNotification($tiraggio, $tiraggio->user));
				Log::channel("trattenuta")->info(
					"Notification sent to user {$tiraggio->user_ndg} for tiraggio {$tiraggio->id}"
				);
			} else {
				Log::channel("trattenuta")->warning(
					"Skipping notification for user {$tiraggio->user_ndg} (filiale role: {$userRoleName})"
				);
			}

			// Always notify filiale (if exists)
			if ($tiraggio->user->filiale) {
				$tiraggio->user->filiale->notify(new FilialeCloseTiraggioNotification($tiraggio, $tiraggio->user->filiale));
			}
		}

		echo "Closed {$tiraggiToClose->count()} tiraggi\n";
	}

	/**
	 * Update tiraggio trattenuta_totale from actual trattenute in DB
	 */
	private static function updateTiraggioTrattenutaFromDB(): void
	{
		// Get all accepted tiraggi
		$tiraggi = Tiraggio::where("status", "accepted")->get();

		if ($tiraggi->isEmpty()) {
			echo "No accepted tiraggi to update\n";
			return;
		}

		echo "Updating tiraggi trattenuta from DB...\n";
		$updated = 0;

		foreach ($tiraggi as $tiraggio) {
			// Skip if already paid
			if ($tiraggio->status === "paid") {
				continue;
			}

			// Calculate actual trattenuta_totale from trattenute
			$actualTrattenutaTotale = \App\Models\Trattenuta::where(
				"tiraggio_numero_pratica",
				$tiraggio->numero_pratica
			)->sum("importo_trattenuto");

			$oldTrattenuta = $tiraggio->trattenuta_totale ?? 0;

			// Only update if there's an actual change
			if (abs($actualTrattenutaTotale - $oldTrattenuta) > 0.01) {
				$tiraggio->trattenuta_totale = $actualTrattenutaTotale;
				$tiraggio->save();
				$updated++;
				Log::channel("trattenuta")->info(
					"Tiraggio {$tiraggio->id} updated: old trattenuta {$oldTrattenuta} → new trattenuta {$actualTrattenutaTotale}"
				);
			}
		}

		echo "Updated {$updated} tiraggi\n";
	}
}
