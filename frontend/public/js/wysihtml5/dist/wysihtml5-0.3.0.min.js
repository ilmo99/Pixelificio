...
window.rangy = (function () {
	...
	if (typeof window == j) f("No window found");
		...
		return (
			b(document, "addEventListener") && document.addEventListener("DOMContentLoaded", s, !1),
...
rangy.createModule("DomUtil", function (b, c) {
	...
	q.areHostMethods(r, ["splitText", "deleteData", "insertData", "appendData", "cloneNode"]) ||
...
rangy.createModule("DomRange", function (b) {
	...
	function j(b) {
		for (var c, d = a(b.range).createDocumentFragment(), h; (c = b.next()); ) {
			b.isPartiallySelectedSubtree()
				? ((c = c.cloneNode(!1)), (h = b.getSubtreeIterator()), c.appendChild(j(h)), h.detach(!0))
				: b.remove();
	...
	function v(a) {
		o(a);
		if (
			(!l.arrayContains(G, a.startContainer.nodeType) && !O(a.startContainer, !0)) ||
		...
		b.util.extend(a.prototype, {
			...
			splitBoundaries: function () {
				...
				l.isCharacterDataNode(a) &&
					...
					e ? ((h -= b), (c = a)) : c == a.parentNode && h >= l.getNodeIndex(a) && h++,
					(b = 0));
				d(this, a, b, c, h);
			},
				...
				var a = this.startContainer,
					...
					e = function (a) {
						...
						b &&
							...
							((c = a), (h = a.length), a.appendData(b.data), b.parentNode.removeChild(b));
					},
					j = function (d) {
						...
						if (e && e.nodeType == d.nodeType) {
							...
							a == c
								? ((h += b), (c = a))
								: c == d.parentNode && ((e = l.getNodeIndex(d)), h == e ? ((c = d), (h = j)) : h > e && h--);
						}
					},
					k = !0;
				l.isCharacterDataNode(c)
					? c.length == h && e(c)
					: (0 < h && (k = c.childNodes[h - 1]) && l.isCharacterDataNode(k) && e(k), (k = !this.collapsed));
				k
					? l.isCharacterDataNode(a)
						? 0 == b && j(a)
						: b < a.childNodes.length && (e = a.childNodes[b]) && l.isCharacterDataNode(e) && j(e)
					: ((a = c), (b = h));
				d(this, a, b, c, h);
			},
...
rangy.createModule("WrappedSelection", function (b, c) {
	...
	function g(a) {
		...
		a instanceof x
			...
			: a instanceof o
				? (c = a.nativeRange)
				: b.features.implementsDomRange && a instanceof m.getWindow(a.startContainer).Range && (c = a);
		return c;
	}
	function i(a) {
		var b = a.getNodes(),
	...
	function p(a, b) {
		for (
			var c = m.getDocument(b[0].startContainer), c = m.getBody(c).createControlRange(), d = 0, e;
			d < rangeCount;
			++d
		) {
			e = i(b[d]);
	...
	F &&
		...
		((u = F.createControlRange()), s.areHostProperties(u, ["item", "add"]) && (l = !0));
					...
					if (l && v && this.docSelection.type == "Control") j(this, a);
						...
						if (this.rangeCount == d + 1) {
							if (b.config.checkSelectionRanges)
								(d = B(this.nativeSelection, this.rangeCount - 1)) && !x.rangesEqual(d, a) && (a = new o(d));
	...
	else if (E && "boolean" == typeof t.isCollapsed && "boolean" == typeof K.collapsed && b.features.implementsDomRange)
		G = function (a) {
			var b;
			b = a.nativeSelection;
			if (b.anchorNode) {
	...
	u.toHtml = function () {
		...
		if (this.rangeCount) {
			for (
				var a = x.getRangeDocument(this._ranges[0]).createElement("div"), b = 0, c = this._ranges.length;
				b < c;
				++b
			)
...
Base.prototype = {
	extend: function (b, c) {
		if (1 < arguments.length) {
			...
			for (
				var f = { toSource: null }, g = ["constructor", "toString", "valueOf"], i = Base._prototyping ? 0 : 1;
				(h = g[i++]);
	...
	return {
		stop: function () {
			for (var e, h = 0, f = c.length; h < f; h++)
				(e = c[h]), b.removeEventListener ? b.removeEventListener(e, a, !1) : b.detachEvent("on" + e, d);
		},
	};
};
		...
		if (!("MENU" !== a.nodeName && "UL" !== a.nodeName && "OL" !== a.nodeName)) {
			...
			for (e && "block" !== b.getStyle("display").from(e) && c(d); (i = a.firstChild); ) {
				for (f = i.lastChild; (e = i.firstChild); )
					(g = (g = e === f) && "block" !== b.getStyle("display").from(e) && "BR" !== e.nodeName),
						d.appendChild(e),
						g && c(d);
...
(function (b, c) {
	...
	f.prototype = {
		...
		postApply: function (a, b) {
			for (
				var c = a[0], d = a[a.length - 1], f = [], g, i = c, m = d, s = 0, x = d.length, o, z, w = 0, y = a.length;
				w < y;
				++w
			)
							...
							for (var e = a.attributes.length, f, g; d < e; ++d)
								if (
									...
									"class" != g &&
										...
										f.specified != g.specified || (f.specified && f.nodeValue !== g.nodeValue)))
								) {
									d = !1;
									break a;
								}
		...
		if (e)
			var f = g.observe(a, "DOMNodeInserted", function (a) {
				...
				a.nodeType === b.ELEMENT_NODE &&
					((c = g.getStyle("display").from(a)), "inline" !== c.substr(0, 6) && (a.className += " " + e));
			});
		a.execCommand(c, !1, d);
		f && f.stop();
	}
		...
		exec: function (h, k, j, n, p) {
			...
			if (r)
				...
				if (null === j || b.lang.array(i).contains(j))
					if (((m = h.selection.getSelectedNode()), (r = g.getParentElement(m, { nodeName: i })))) {
						h.selection.executeAndRestoreSimple(function () {
							j && (r = g.renameElement(r, j));
							if (n) {
								var a = r;
								a.className
									? ((a.className = a.className.replace(p, "")), (a.className += " " + n))
									: (a.className = n);
							}
						});
						return;
	...
	b.commands.formatInline = {
		...
		state: function (d, f, g, i, h) {
			var f = d.doc,
				k = a[g] || g;
			if (
				(!b.dom.hasElementWithTagName(f, g) && !b.dom.hasElementWithTagName(f, k)) ||
	...
	b.views.Composer.prototype.observe = function () {
		...
		c.observe(i, "mouseover", function (a) {
			...
			!("A" !== b && "IMG" !== b) &&
				...
				((b = n[b] + (a.getAttribute("href") || a.getAttribute("src"))), a.setAttribute("title", b));
		});
	};
})(wysihtml5);
(function (b) {
...
(function (b) {
	...
	b.toolbar.Toolbar = Base.extend({
		constructor: function (a, c) {
			...
			this.show();
			for (
				var e = this.container.querySelectorAll("[data-wysihtml5-command=insertSpeech]"), f = e.length, g = 0;
				g < f;
				g++
			)
		...
		_getLinks: function (a) {
			for (
				...
				(i = c[f]),
					...
					(g[k + ":" + j] = { link: i, group: h, name: k, value: j, dialog: n, state: !1 });
		},
				...
				if (
					...
					(g.state = f)
						? (c.addClass(g.link, "wysihtml5-command-active"),
							...
							g.dialog && ("object" === typeof f ? g.dialog.show(f) : g.dialog.hide()))
						: (c.removeClass(g.link, "wysihtml5-command-active"),
							g.group && c.removeClass(g.group, "wysihtml5-command-active"),
							g.dialog && g.dialog.hide());
			for (e in d)
